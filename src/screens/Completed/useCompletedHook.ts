...
// Hook personalizado para manejar la lógica de la pantalla de tareas completadas
export const useCompletedHook = () => {
  const {token} = useAuth(); // Obtiene el token de autenticación del contexto
  // Llama al hook para obtener tareas desde el endpoint de tareas completadas
  const {tasks, loading, error, fetchTasks} = useFetchTasks(
    `${Config.HOST}/api/tasks/completed`,
  );

  const [refreshing, setRefreshing] = useState<boolean>(false); // Estado para manejar el refresco de la lista de tareas
  ...
  const handleUpdateTaskStatus = async (
    ...
    status: 'todo' | 'inprogress', // Nuevo estado de la tarea
  ) => {
    try {
  ...
  const actions = [
    ...
    {
      label: 'IN PROGRESS', // Etiqueta para marcar como "en progreso"
      onPress: () =>
        handleUpdateTaskStatus(selectedTask?.taskId ?? '', 'inprogress'), // Actualiza el estado al presionar
    },
