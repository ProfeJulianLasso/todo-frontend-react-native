...
// Hook personalizado para manejar la lógica de la pantalla de tareas por hacer
export const useToDoHook = () => {
  const {token} = useAuth(); // Obtiene el token de autenticación del contexto
  // Llama al hook para obtener tareas desde el endpoint de tareas por hacer
  const {tasks, loading, error, fetchTasks} = useFetchTasks(
    `${Config.HOST}/api/tasks/todo`,
  );
  ...
  const handleUpdateTaskStatus = async (
    ...
    status: 'inprogress' | 'completed', // Nuevo estado de la tarea
  ) => {
    try {
